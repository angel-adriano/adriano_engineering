...
try:
	...
	_logger.error('**Install ShopifyApi python package cmd: `pip3 install ShopifyApi==8.0.0`')
if shopify.VERSION != '8.0.0':
	_logger.error('**ShopifyApi v8.0.0 required. `pip3 install ShopifyApi==8.0.0`')
...
class Bridge:
	def __init__(self,url,api_key,password,channel_id=False,pricelist_id=False,**kw):
		...
		self.url          = '{}/admin/api/{}'.format(url, API_VERSION)
	...
	def process_product(self,product):
		...
		if isinstance(product.body_html, str):
	...
	def get_orders(self,**kw):
		...
		if kw.get('object_id'):
			...
			options = self.pre_get(kw)
			if options.get('limit'):
				orders = self.ensure_response('Order','find',**options)
				for order in orders:
					if order.financial_status != 'refunded':
						order_data_list.append(self.process_order(order))
	...
	def put_variant(self,record,remote_id,product):
		...
		variant.price                = round(record.with_context(pricelist=self.pricelist_id).price,2)
